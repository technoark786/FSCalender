...
- (void)layoutSubviews
{
    ...
    int tCount = self.numberOfEvents;
    if (self.numberOfEvents > 2) {
        tCount = 3;
    }
    ...
    self.contentView.fs_width = (tCount*2-1)*diameter; //(self.numberOfEvents*2-1)*diameter
    ...
    if (layer == self.layer) {
        ...
        for (int i = 0; i < self.eventLayers.count; i++) {
            ...
            if (!eventLayer.hidden) {
                if (i > 2) {
                    eventLayer.frame = CGRectMake(2*tYPos*diameter, (self.fs_height-diameter) + 2, diameter, diameter);
                    
                }else{
                    eventLayer.frame = CGRectMake(2*tYPos*diameter, (self.fs_height-diameter)-5, diameter, diameter);
